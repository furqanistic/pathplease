...
class PathPleaseManager {
    ...
    registerCommands(context) {
        const commands = [
            vscode.commands.registerCommand('pathplease.addPath', () => this.addPathComment()),
            vscode.commands.registerCommand('pathplease.removePath', () => this.removePathComment()),
            vscode.commands.registerCommand('pathplease.toggleAutoAdd', () => this.toggleAutoAdd()),
            vscode.commands.registerCommand('pathplease.refreshPath', () => this.refreshPathComment()),
            vscode.commands.registerCommand('pathplease.showOutput', () => this.outputChannel.show()),
        ];
    ...
    shouldAutoProcess(document) {
        ...
        if (!this.canHaveComments(document.languageId, fileName)) {
            this.log(`Auto-process skipped: ${fileName} - file type doesn't support comments`);
            return false;
        }
        // Check if path comment already exists
        if (this.hasPathComment(document)) {
            this.log(`Auto-process skipped: ${fileName} - path comment already exists`);
            return false;
        }
        return true;
    }
    ...
    async getDisplayPath(document) {
        ...
        switch (config.pathFormat) {
            ...
            default:
                if (workspaceFolder) {
                    let relativePath = path.relative(workspaceFolder.uri.fsPath, document.uri.fsPath);
    ...
    async updatePathComment(editor, newPath, style) {
        ...
        if (commentRange) {
            ...
            await editor.edit((editBuilder) => {
                editBuilder.replace(new vscode.Range(commentRange.start.line, 0, commentRange.start.line, doc.lineAt(commentRange.start.line).text.length), newComment);
            });
        }
    }
    ...
    async toggleAutoAdd() {
        ...
        await config.update('autoAddOnOpen', !currentValue, vscode.ConfigurationTarget.Global);
